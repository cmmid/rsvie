...
efficacy_t <- load(file = system.file(package = "rsvie", "extdata", "efficacies", "mat_post_wane.RData")) 
...
immune_profile_mat <- list(
    mass = list(
        ...
        wane_function_neonate = "a * exp(b * t))", a_mat = abs(efficacy_mat$wane_a), b_mat = abs(efficacy_mat$wane_b)
    ),
    vhr = list(
        product = "pal",
...
RSV_mat_s <- run(RSV_mat_s)
RSV_mat_yr <- run(RSV_mat_yr)
...
RSV_mat_s_states %>% filter(state_name %in% c("sum18", "sum19", "sum20", "sum21", "sum22", "sum23" )) %>% 
